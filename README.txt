INTERNET SKILLS HUB
===================

To use the site:
1. Open index.html in a web browser.
2. No internet connection or installation is required.

To change the learning content:
- data/glossary-data.js contains the glossary terms.
- data/quiz-data.js contains the multiple-choice questions.
- data/scenario-data.js contains the Safe or Risky cards.

The matching game automatically uses entries from glossary-data.js.
Learner progress and theme choice are saved only in the current browser.

